...
# --- 해상도 체크 함수 ---
def check_image_resolution(image, min_width=720, min_height=1280):
    """이미지의 가로/세로 중 짧은 쪽이 720px, 긴 쪽이 1280px 미만이면 False를 반환합니다."""
    width, height = image.size
    short_side = min(width, height)
    long_side = max(width, height)
    
    if short_side < min_width or long_side < min_height:
        st.error(f"이미지 해상도가 너무 낮습니다. (현재: {width}x{height}, 최소: {min_width}x{min_height})")
        return False
    return True
...
def analyze_and_display_results(image):
    ...
    with st.spinner('이미지를 분석 중입니다...'):
        ...
        st.image(annotated_image_rgb, caption="탐지 결과", use_container_width=True)
...
with tab1:
    ...
    st.info("""
    **촬영 가이드**\n
    - **권장 해상도**: 1080×1920 (FHD) 이상
    - 웹캠을 이용해 피부 사진을 찍으면, AI가 자동으로 분석하여 결과를 보여줍니다.
    - 얼굴이나 피부 부위가 흐릿하지 않도록 촬영해 주세요.
    - 과도한 필터, 보정, 플래시 사용은 피해주세요.
    - 웹캠을 이용해 촬영시 빛 번짐, 그늘짐에 주의하세요. 분석 정확도가 떨어지게 되는 요인이 됩니다.
    """, icon="📸")
    st.warning("⚠️ 웹캠 촬영은 편의 기능으로, 해상도가 낮아 분석 정확도가 떨어질 수 있습니다. **정확한 분석을 원하시면 '이미지 업로드 분석' 탭을 이용해 주세요.**")
    ...
    if picture is not None:
        ...
        # 웹캠은 해상도가 낮게 고정되므로, 해상도 체크를 생략하고 바로 분석
    ...
    **이미지 업로드 가이드**\n
    ...
    if uploaded_file is not None:
        ...
        if check_image_resolution(image):
            ...
            st.warning("재업로드하세요. 해상도가 너무 낮습니다.")
